        ...
        <div v-if="!selectedElement.multiple" class="mb-3 mt-4 ml-4">
          <div v-for="(option, index) in selectedElement.options" :key="index" class="mb-2">
            <div class="input-group">
              <input :id="'option-' + index" type="radio" :name="'singleChoiceGroup'" :value="option.id"
...
watch(selectedElement, (newElement) => {
  if (newElement) {
    studentAnswers.value = newElement.multiple ? [] : null;
  }
});
...
watch(
  ...
  (newElement) => {
    if (newElement) {
      // Réinitialiser studentAnswers
      studentAnswers.value = newElement.multiple ? [] : null;

      // Si l'élément sélectionné change, mettre à jour Quill
...
const submitAnswers = () => {
  ...
  selectedElement.value.options.forEach((option) => {
    if (studentAnswers.value.includes(option.id)) {
      results.value.selectedOptions.push(option.text); // Ajouter la réponse choisie
      if (option.isCorrect) {
        results.value.score += parseInt(option.weight); // Bonne réponse
      } else {
        if (option.weight == 0 ||option.weight==null ||option.weight.length == 0   ) { option.weight = 0 }
        results.value.error += parseInt(option.weight); // Mauvaise réponse
      }
    }
  });
  ...
  results.value.mark = (results.value.score + results.value.error / maxScore).toFixed(2); // Note en pourcentage

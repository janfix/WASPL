...
const fetchTestsForTabulator = async (url, config, params) => {
  ...
  try {
    console.log("URL avec filtres:", `${VITE_API_BASE_URL}/api/tests/getTests${queryParams}`);
    const response = await axios.get(`${VITE_API_BASE_URL}/api/tests/getTests${queryParams}`);
    return {
...
const initTable = () => {
  table = new Tabulator("#test-table", {
    ...
    ajaxURL: `${VITE_API_BASE_URL}/api/tests/getTests`,

...
const handleImport = async () => {
  ...
  try {
    ...
    for (const student of studentsPreview.value) {
      ...
      const existingStudent = await axios.get(`${VITE_API_BASE_URL}/api/students/check?email=${student.email}`);
      ...
      const response = await axios.post(`${VITE_API_BASE_URL}/api/students`, student);
    ...
    if (groupOption.value === "new") {
      const groupResponse = await axios.post(`${VITE_API_BASE_URL}/api/groups`, {
     ...
     if (groupOption.value === "existing") {
      await axios.put(`${VITE_API_BASE_URL}/api/groups/${selectedGroup.value}/add-students`, {
    ...
    for (const studentId of studentIds) {
      await axios.post(`${VITE_API_BASE_URL}/api/students/${studentId}/add-group`, { groupId });
    }
